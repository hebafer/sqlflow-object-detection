...
# Inference
def detect(model, image_path, tasks, latency, percentage, count=0, names=[]):
...
def inference():
    ...
    query_parameters = pd.read_csv('/opt/sqlflow/datasets/model_config_task_all.csv', index_col='index').loc[(args.experiment_index)]
    ...
    for row in result_df.itertuples():
        detected_objects = detect(model, row.file_name, tasks=tasks,
                                         latency=latency, percentage=percentage, names=categories)
